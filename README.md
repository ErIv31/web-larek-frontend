...
- `formPaymentAddress: change` - изменение формы с выбором способа оплаты и адресом;
...
- `formEmailPhone: change` - изменение формы с контактной информацией пользователя;
- `formEmailPhone: submit` - подтверждение формы с контактной информацией пользователя и отправка данных на сервер;
